...
import { z } from 'zod';
import { Plus, Briefcase, FileText, Upload, Download, ChevronRight, X } from 'lucide-react';
...
export function HiringPage() {
  ...
  const createJob = useMutation({
    ...
    onSuccess: () => { reset(); qc.invalidateQueries({ queryKey: ['jobs'] }); showToast('Job posting created!', 'success'); },
  ...
  const updateAppStatus = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['applications'] }); showToast('Status updated', 'success'); },
    onError: (e) => showToast(getErrorMessage(e), 'error'),
  });
  ...
  const toggleJob = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['jobs'] }); showToast('Job status updated', 'success'); },
    onError: (e) => showToast(getErrorMessage(e), 'error'),
  });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'HIRED': return <Badge className="bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 border-emerald-500/20">HIRED</Badge>;
      case 'REJECTED': return <Badge className="bg-red-500/10 text-red-600 hover:bg-red-500/20 border-red-500/20">REJECTED</Badge>;
      case 'INTERVIEW': return <Badge className="bg-indigo-500/10 text-indigo-600 hover:bg-indigo-500/20 border-indigo-500/20">INTERVIEW</Badge>;
      default: return <Badge className="bg-amber-500/10 text-amber-600 hover:bg-amber-500/20 border-amber-500/20">PENDING</Badge>;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">Recruitment & Hiring</h2>
        <p className="text-muted-foreground mt-2">Manage job postings and review candidate applications.</p>
      </div>
      ...
      <div className={`grid gap-6 ${isAdmin ? 'lg:grid-cols-3' : 'grid-cols-1'}`}>
        ...
        <div className="space-y-6 lg:col-span-1">
          {isAdmin && (
            <Card className="bg-white/50 backdrop-blur-xl">
              <CardHeader>
                ...
                <CardDescription>Create a new opening on the job board.</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit((v) => createJob.mutate(v))} className="space-y-4">
                  <div className="space-y-2">
                    <Label>Job Title</Label>
                    <Input placeholder="e.g. Senior Engineer" {...register('title')} className="bg-white/50" />
                    {errors.title && <p className="text-red-500 text-xs">{errors.title.message}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label>Department</Label>
                    <Input placeholder="e.g. Engineering" {...register('department')} className="bg-white/50" />
          ...
          <Card className="bg-white/50 backdrop-blur-xl overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between pb-4">
              ...
              <Badge variant="secondary" className="bg-indigo-50 text-indigo-600 font-medium border-0">
                {jobs.data?.filter(j => j.status === 'OPEN').length ?? 0} active
              </Badge>
            </CardHeader>
            <div className="divide-y divide-border/50">
              {jobs.isLoading && <div className="p-6 text-center text-muted-foreground">Loading...</div>}
              {jobs.data?.map((j) => (
                <div 
                  key={j.id}
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      {j.status === 'OPEN' ? (
                        <Badge className="bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 border-0 h-5 px-1.5 text-[10px]">OPEN</Badge>
                      ) : (
                        <Badge variant="secondary" className="h-5 px-1.5 text-[10px]">CLOSED</Badge>
                      )}
                  ...
                  {selectedJobId === j.id && isAdmin && (
                    <div className="flex items-center gap-2 mt-4 pt-4 border-t border-border/50" onClick={(e) => e.stopPropagation()}>
                      <Button 
                        size="sm" 
                        variant="outline"
                        className={j.status === 'OPEN' ? 'text-red-500 hover:text-red-600 hover:bg-red-50' : 'text-emerald-500 hover:text-emerald-600 hover:bg-emerald-50'}
        ...
        <Card className="lg:col-span-2 bg-white/50 backdrop-blur-xl overflow-hidden h-fit">
          <CardHeader className="flex flex-row items-center justify-between pb-4">
            <div>
              <CardTitle>{selectedJob ? `Applications — ${selectedJob.title}` : 'All Applications'}</CardTitle>
            </div>
            <div className="flex items-center gap-2">
              {selectedJobId && (
          ...
          <form onSubmit={submitApplication} className="space-y-4 pt-2">
            {!applyJobId && (
              <div className="space-y-2">
                ...
                <select name="jobId" required className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2">
            ...
            <div className="space-y-2 pb-4">
              ...
              <div className="border-2 border-dashed border-indigo-200 hover:border-indigo-400 bg-indigo-50/30 rounded-xl p-8 text-center cursor-pointer transition-colors"
                onClick={() => resumeRef.current?.click()}>
                <Upload size={24} className="mx-auto mb-3 text-indigo-400" />
                <div className="text-sm font-medium text-slate-700">Click to upload resume/documents</div>
                <div className="text-xs text-muted-foreground mt-1">PDF, DOC, DOCX, JPG, PNG · Max 10MB</div>
                <div className="text-sm font-semibold text-indigo-600 mt-2 empty:hidden"></div>
                <input ref={resumeRef} name="resume" type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" className="hidden"
                  onChange={(e) => { const name = e.target.files?.[0]?.name; if (name) (e.target.parentElement!.querySelector('div:last-of-type') as any).textContent = `📎 ${name}`; }} />
              </div>
            </div>
